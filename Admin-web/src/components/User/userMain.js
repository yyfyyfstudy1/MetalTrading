...
export default {
    ...
    methods: {
        ...
        confirmDelete(row) {
            this.$confirm('Are you sure you want to send a password change email ?', 'Warning', {
                confirmButtonText: 'OK',
                cancelButtonText: 'Cancel',
                type: 'warning'
            }).then(() => {
                this.deleteProduct(row.email);
            }).catch(() => {
                console.log('Delete action canceled');
            });
        },
        deleteProduct(userEmail) {

            const requestBody = {
                emailAddress: userEmail
            }
            this.$axios.post(this.$httpurl + '/public/forgetPassword',  requestBody )
                .then(res => res.data)
                .then(res => {

                    if (res.code === 200) {
                        this.loadGet();
                        this.$message.success("Send reset password Email successful");
        ...
        prepareDataForBackend(frontendData) {
            ...
            const backendData = {
                ...
                name: frontendData.name, // 或者 frontendData.category，根据后端的实际要求
                email: frontendData.email,
                gender: frontendData.gender,
                activationStatus: frontendData.activationStatus
        ...
        actualSaveProduct() {
            ...
            this.$axios.post(this.$httpurl + '/public/admin/adminUploadUserInfo', this.prepareDataForBackend(this.editingProduct), config)
                .then(res => res.data)
                .then(res => {
                    console.log(res);
                    if (res.code === 200) {
        ...
        filterStatus(value, row) {
            ...
            return row.activationStatus == value;
        },
        genderStatus(value, row){
        ...
        loadGet() {
           const queryParams = {
            ...
            this.$axios.get(this.$httpurl + '/public/admin/getUserListAdmin', { params: queryParams }, config)
                .then(res => res.data)
                .then(res => {
